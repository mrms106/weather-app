...
function showData(city) {
  getWeatherData(city, (error,result) => {
    if (error) {
      console.error('Error occurred:', error);
      // Send an error response
      locationElement.textContent = "An error occurred while fetching weather data.";
  } else {
    console.log(result);
    if (result.cod == 200) {
      if (
...
function getWeatherData(city, callback) {
  ...
  fetch(locationApi)
        ...
        .catch((error) => {
            ...
            callback(error, null); // Pass an error object to the callback
        });
}